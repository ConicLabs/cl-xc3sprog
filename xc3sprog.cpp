...
Copyright (C) 2004 Andrew Rogers
              2005-2011 Uwe Bonnes bon@elektron.ikp.physik.tu-darmstadt.de
							2015 Ali Asgar Sohanghpurwala asohangh@conic-labs.com, Consolidated Logic Inc.
...
int programXC3S(Jtag &g, int argc, char **args, bool verbose,
                bool reconfig, bool partial, int family);
...
int main(int argc, char **args)
{
  ...
  while(true) {
      int c = getopt(argc, args, "?hCaLc:d:DeE:F:i:I::jJ:Lm:o:p:Rs:S:T::vX:");
    switch(c) 
    {
    case -1:
      goto args_done;
    ...
    case 'a':
			printf("Active Reconfig Selected\n");
			partial = true;
      break;
...
int programXC3S(Jtag &jtag, int argc, char** args,
                bool verbose, bool reconfig, bool partial, int family)
{
  ...
  else
  {
      for(i=0; i< argc; i++)
      {
          ...
          BitFile bitfile;
					...
					// This is nasty, but should work for now
					if(!strcmp(args[i],"-")) {
						printf("Programming from stdin\n");
						if(partial) {
							return alg.flow_program_stdin();
						}
						else {
							return alg.array_program_stdin();
						}
					}
          ...
          if(verbose) 
          {
              ...
              fprintf(stderr, "Bitstream length: %u bits\n",
					...
					if(partial) {
						alg.partial_array_program(bitfile);
					}
					else {
               ...
               char *bscanfile, int family, const char *device)
{
    ...
    if (bscanfile)
    {
        programXC3S(jtag, 1, &bscanfile, verbose, 0, 0, family);
    }
